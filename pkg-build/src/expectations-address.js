/*!
 * expectations-address v1.0.0 | (c) 2025 RexxJS Project | MIT License
...
const MATCHERS = {
  ...
  equality: {
    ...
    message: (actual, expected, negated) =>
      negated ? `${expected} not expected, but encountered`
              : `${expected} expected, but ${actual} encountered`
  },
    ...
    switch (commandOrMethod.toLowerCase()) {
      ...
      default:
        ...
        if (typeof commandOrMethod === 'string' && commandOrMethod.includes('should')) {
          // For REXX ADDRESS context, let expectation failures throw to terminate script
