...
async fn start_http_server(state: Arc<ServerState>, port: u16) {
    let app = Router::new()
        .route("/api/etch", post(handle_control_bus))
    ...
    log::info!("Rexx-A-Sketch Control Bus HTTP API listening on http://{}", addr);
  ...
  tauri::Builder::default()
    .setup(move |app| {
      ...
      if enable_http {
          ...
          log::info!("Control Bus HTTP API enabled on port 8084");
          log::info!("Auth token: {}", auth_token);
      }

      // Emit init message to frontend
      let window = app.get_webview_window("main").unwrap();
      std::thread::spawn(move || {
        std::thread::sleep(std::time::Duration::from_millis(500));
        window.emit("init", InitPayload {
          message: "Hey, Etch. Draw!".to_string()
        }).unwrap();
      });
