/*!
 * rexxjs/provisioning-shared-utils v1.0.0 | (c) 2025 RexxJS Project | MIT License
 * @rexxjs-meta=PROVISIONING_SHARED_UTILS_META
 */
/**
 ...
 *
 * This is a shared library module that provides common functionality.
 * It should be REQUIRE'd by address handlers that need these utilities.
 */

// Try to import RexxJS interpolation config
let interpolationConfig = null;
try {
  interpolationConfig = require('../../core/src/interpolation-config.js');
} catch (e) {
  // Not available - will use default interpolation
}
...
async function interpolateMessage(template, context = {}) {
  ...
  // Use configured interpolation pattern if available
  if (interpolationConfig && interpolationConfig.getPattern) {
    const pattern = interpolationConfig.getPattern();
...
// Metadata function for RexxJS module detection
if (typeof global !== 'undefined') {
  global.PROVISIONING_SHARED_UTILS_META = function() {
    return {
      canonical: 'org.rexxjs/provisioning-shared-utils',
      type: 'shared-library',
      name: 'Provisioning Shared Utilities',
      version: '1.0.0',
      description: 'Shared utilities for container and VM address handlers',
      provides: {
        utilities: [
          'interpolateMessage', 'createLogFunction', 'logActivity',
          'parseCommand', 'parseCommandParts', 'parseMemoryLimit',
          'testRuntime', 'validateCommand', 'validateVolumePath',
          'validateBinaryPath', 'auditSecurityEvent', 'calculateUptime',
          'parseKeyValueString', 'parseCheckpointOutput',
          'wrapScriptWithCheckpoints', 'parseEnhancedCheckpointOutput',
          'formatStatus'
        ]
      },
      dependencies: {},
