    ...
    compiler.hooks.emit.tap('RexxMetaPlugin', (compilation) => {
      Object.keys(compilation.assets).forEach(filename => {
        if (filename.endsWith('.bundle.js')) {
          ...
          // Update the asset (deprecated but working)
...
module.exports = {
  ...
  entry: path.resolve(__dirname, 'graphviz-functions.js'),
  output: {
    filename: 'graphviz-functions.bundle.js',
    path: path.resolve(getGitRoot(), '../dist/functions'),
    library: 'graphviz-functions',
    libraryTarget: 'umd',
    globalObject: 'this'
  },
  externals: {
