...
class AddressProxmoxHandler {
  constructor() {
    ...
    this.baseContainers = new Map(); // baseName -> { vmid: vmid, created: timestamp }
  }
  ...
  async handleAddressCommand(command, context = {}) {
    try {
      ...
      switch (parsed.operation) {
        ...
        case 'register_base': return this.registerBase(parsed.params);
        case 'clone': return this.cloneContainer(parsed.params);
        case 'clone_from_base': return this.cloneFromBase(parsed.params);
        case 'list_bases': return this.listBases();
  ...
  /**
   * Register a container as a base for CoW cloning (converts to template)
   */
  async registerBase(params) {
    const { name, vmid } = params;
    if (!name) throw new Error('Missing required parameter: name (base name)');

    // If vmid provided, use it; otherwise use name as both
    const containerVmid = vmid || name;
    if (!this.activeContainers.has(String(containerVmid))) {
      throw new Error(`Container not found: ${containerVmid}`);
    }

    const info = this.activeContainers.get(String(containerVmid));

    // Stop container before converting to template
    if (info.status === 'running') {
      await this.execPct(['stop', String(containerVmid)]);
      info.status = 'stopped';
    }

    // Convert to template using pct template command
    // This makes it a CoW base - clones will use linked storage
    const res = await this.execPct(['template', String(containerVmid)]);
    if (res.exitCode !== 0) throw new Error(`pct template failed: ${res.stderr || res.stdout}`);

    // Register as base
    this.baseContainers.set(name, {
      vmid: String(containerVmid),
    ...
    return {
      ...
      baseName: name,
      vmid: String(containerVmid),
      output: `Container ${containerVmid} registered as template base: ${name}`
    };
  }

  /**
   * Clone a container using Proxmox linked clone (CoW)
   */
  async cloneContainer(params) {
    const { source, vmid, hostname } = params;
    if (!source) throw new Error('Missing required parameter: source (source VMID)');

    const sourceVmid = String(source);
    if (!this.activeContainers.has(sourceVmid)) {
      throw new Error(`Source container not found: ${sourceVmid}`);
    }
    ...
    const sourceInfo = this.activeContainers.get(sourceVmid);

    // Generate new VMID
    const newVmid = vmid ? String(vmid) : String(++this.containerCounter);
    if (this.activeContainers.has(newVmid)) {
      throw new Error(`VMID already exists: ${newVmid}`);
    }

    const startTime = Date.now();

    // Use pct clone - Proxmox's CoW cloning feature
    // When source is a template, this creates a linked clone (CoW)
    const cloneArgs = ['clone', sourceVmid, newVmid];
    if (hostname) cloneArgs.push('--hostname', hostname);

    const res = await this.execPct(cloneArgs);
    if (res.exitCode !== 0) throw new Error(`pct clone failed: ${res.stderr || res.stdout}`);
    ...
    const cloneInfo = {
      vmid: newVmid,
      template: sourceInfo.template,
      storage: sourceInfo.storage,
      status: 'stopped',
      created: new Date().toISOString(),
      clonedFrom: sourceVmid,
      cloneMethod: sourceInfo.isTemplate ? 'linked-clone' : 'full-clone'
    };

    this.activeContainers.set(newVmid, cloneInfo);
    ...
    return {
      ...
      method: cloneInfo.cloneMethod,
      output: `Container ${newVmid} cloned from ${sourceVmid} in ${cloneTimeMs}ms using ${cloneInfo.cloneMethod}`
    };
  }

  /**
   * Clone from registered base
   */
  async cloneFromBase(params) {
    const { base, vmid, hostname } = params;
    if (!base) throw new Error('Missing required parameter: base');

    const baseInfo = this.baseContainers.get(base);
    if (!baseInfo) {
      throw new Error(`Base not found: ${base}. Use register_base first.`);
    }

    // Clone from the base container
    return await this.cloneContainer({ source: baseInfo.vmid, vmid, hostname });
  }
   ...
   */
  async listBases() {
    const bases = Array.from(this.baseContainers.entries()).map(([name, info]) => ({
      name,
      vmid: info.vmid,
    ...
    return {
      ...
      bases,
      count: bases.length,
      output: `Found ${bases.length} registered base templates`
    };
  }
...
const ADDRESS_PROXMOX_METHODS = {
  ...
  'register_base': 'Register container as template base for CoW cloning [name=] [vmid=]',
  'clone': 'Clone container using Proxmox linked clone (CoW) [source=] [vmid=] [hostname=]',
  'clone_from_base': 'Clone from registered base template [base=] [vmid=] [hostname=]',
  'list_bases': 'List registered base templates'
};
