/**
 * ADDRESS NSPAWN Handler Tests - Complete Test Suite
...
describe('ADDRESS NSPAWN Handler - Full Suite', () => {
  ...
  async function createMockHandler(config = {}) {
    return await createNspawnTestHandler(config);
  }
  ...
  describe('Initialization', () => {
    test('should initialize with default configuration', async () => {
      ...
      expect(handler.runtime).toBe('systemd-nspawn');
  ...
  describe('Status Operations', () => {
    test('should return status information', async () => {
      ...
      expect(result.runtime).toBe('systemd-nspawn');
  ...
  describe('Container Lifecycle', () => {
    ...
    test('should auto-generate container name if not provided', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.operation).toBe('create');
      expect(result.container).toMatch(/^nspawn-container-\d+$/);
    });
  ...
  describe('Error Handling', () => {
    test('should handle unknown commands gracefully', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain('Unknown ADDRESS NSPAWN command: unknown-command');
    });
    ...
    test('should handle malformed commands', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain('Unknown ADDRESS NSPAWN command:');
    });
    ...
    test('should handle complex command parsing edge cases', async () => {
      ...
      expect(result1.error).toContain('Unknown ADDRESS NSPAWN command:');
      
      // Test command with just spaces and equals
      const result2 = await handler.handleAddressCommand('   =  ');
      expect(result2.success).toBe(false);
      expect(result2.error).toContain('Unknown ADDRESS NSPAWN command:');
    });
  });
  ...
  describe('Container Management Edge Cases', () => {
    ...
    test('should handle auto-generated names with consistent counter', async () => {
      ...
      expect(result1.container).toBe('nspawn-container-1');
      expect(result2.container).toBe('nspawn-container-2');
      expect(result3.container).toBe('nspawn-container-3');
    });
  });
  ...
  describe('Edge Cases and Complex Parsing', () => {
    ...
    test('should handle empty parameter values', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.container).toMatch(/^nspawn-container-\d+$/); // Should auto-generate
    });
